...
static t_bool	f_perform_hexa(t_sstream *ss, long long i, int j, t_bool first)
{
	char	val;

	val = ((i >> (sizeof(long long) * 8 - 4)) & 0xF);
	val = (ss->v_upper && val > 9) ? val + 6 : val;
	if (!first || (first && (val || j == sizeof(long long) * 2 - 1)))
	{
...
static void		f_fill_zero(t_sstream *ss, long long i, size_t n)
{
...
{
	const size_t	n = ft_sstream_count_hexa(ss, i, sizeof(long long));
	t_bool			first;
	size_t			j;
	size_t			reserve;
